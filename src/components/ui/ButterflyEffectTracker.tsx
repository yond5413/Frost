...
function formatChoiceId(choiceId: string): string {
  return choiceId.replace(/^ai_/, '').replace(/_/g, ' ').trim() || 'unknown choice';
}
...
export default function ButterflyEffectTracker() {
  ...
  return (
    <div className="fixed right-0 top-1/2 -translate-y-1/2 z-40 flex items-stretch">
      {/* Expanded panel */}
      {isExpanded && (
        <div className="w-72 max-h-[65vh] flex flex-col bg-black/90 border-l border-white/10 overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
            ...
            <span className="text-[9px] text-gray-600">
              {storyMemory.length} event{storyMemory.length !== 1 ? 's' : ''}
            </span>
          </div>
            ...
            {storyMemory.slice().reverse().map((entry, i) => (
              <div key={`${entry.choiceId}-${entry.sceneId}-${i}`} className="border-l border-white/10 pl-2">
                <div className="flex gap-1.5 mb-1.5 flex-wrap">
                  <span className="text-[9px] uppercase tracking-[0.1em] px-1.5 py-0.5 border border-cyan-900/40 text-cyan-300/80">
                    scene: {entry.sceneId}
                  </span>
                  <span className="text-[9px] uppercase tracking-[0.1em] px-1.5 py-0.5 border border-white/10 text-gray-400">
                    cause: {formatChoiceId(entry.choiceId)}
                  </span>
                </div>

                <p className="text-gray-400 text-xs leading-relaxed">
                  effect: {entry.consequence || 'A consequence was recorded, but details were lost in the storm.'}
                </p>
      ...
      <button
        ...
        aria-label={isExpanded ? 'Collapse butterfly ledger' : 'Expand butterfly ledger'}
      >
        <svg
          viewBox="0 0 100 100"
